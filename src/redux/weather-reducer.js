  ...
  switch (action.type) {
    case 'WEATHER/SET-TODAY-WEATHER':
      return {
        ...
        cityCoord: [
          action.payload.data.city.coord.lat,
          action.payload.data.city.coord.lon,
        ],
...
export const actions = {
  setTodayWeather: (todayWeatherData) => ({
    type: 'WEATHER/SET-TODAY-WEATHER',
    payload: todayWeatherData,
  }),
  setCurrentWeather: (dayTimeNum) => ({
    type: 'WEATHER/SET-CURRENT-WEATHER',
    payload: dayTimeNum,
  }),
  setDailyForecast: (dailyForecastData) => ({
    type: 'WEATHER/SET-DAILY-FORECAST',
    payload: dailyForecastData,
  }),
};
